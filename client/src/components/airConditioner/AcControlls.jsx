...
const mqttOptions={
  ...
  useEffect(() => {
    const mqttClient = mqtt.connect('wss://attendance.thecloudserv.com:9001/ws', {
      clean: false,
      clientId: `mqtt_${Math.random().toString(16).slice(3)}`,
      reconnectPeriod: 1000,
      keepalive: 60,
  ...
  // };
// Handle power toggle with a unified topic
const handlePowerToggle = () => {
  const newPowerState = !power;  // Toggle the power state
  setPower(newPowerState);

  // Define the topic for power control
  const topic = `thecldiot/head_office_b17_b1/it_office_ac/power`;

  // Define the payload based on the new state (ON or OFF)
  const payload = newPowerState ? 'ON' : 'OFF';

  // Send the MQTT message to the unified topic with the payload
